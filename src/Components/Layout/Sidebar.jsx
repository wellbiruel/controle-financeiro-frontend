...
const MENU = [
  {
    ...
    itens: [
      ...
      { label: 'Saídas',   path: '/saidas',     icon: 'M20 12l-1.41-1.41L13 16.17V4h-2v12.17l-5.58-5.59L4 12l8 8 8-8z' },
    ],
  },
  {
    grupo: 'Planejamento',
    itens: [
      { label: 'Fluxo Anual', path: '/fluxo-anual', icon: 'M3 3v18h18V3H3zm8 16H5v-6h6v6zm0-8H5V5h6v6zm8 8h-6v-6h6v6zm0-8h-6V5h6v6z' },
  ...
  },
  {
    grupo: 'Análise',
    itens: [
      { label: 'Relatórios',   path: '/relatorios',  icon: 'M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z' },
      { label: 'Importar Dados', path: '/importacao', icon: 'M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z' },
    ],
  },
];

const SISTEMA = { label: 'Configurações', path: '/configuracoes', icon: 'M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23.41-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z' };
  ...
  return (
    <aside className={`sidebar ${expanded ? 'expanded' : 'collapsed'}`}>
      ...
      <nav className="sb-nav">
        ...
        {/* Configurações */}
        <div
          className={`sb-item ${isActive(SISTEMA.path) ? 'active' : ''}`}
          onClick={() => navigate(SISTEMA.path)}
          title={!expanded ? SISTEMA.label : undefined}
        >
